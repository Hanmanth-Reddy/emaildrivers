<?php
	...
	if(mysql_num_rows($dres)>0)
	{
            ...
            while($drow=mysql_fetch_row($dres))
            {
                ...
                if($previous_processingecampaigncount==0){
                    ...
                    if(mysql_num_rows($ubres)>0)
                    {
                        ...
                        while($ubrow=mysql_fetch_row($ubres))
                        {
                            ...
                            if($campaignlist!="")
                            {
								...
								$que2="update recipient_info set mail_processedtime=NOW() where mailid IN (".$campaignlist.")";
                                mysql_query($que2,$db);
                            }
                            ...
                            if($postinglist!="")
                            {
								...
								$que2="update recipient_info set mail_processedtime=NOW() where mailid IN (".$postinglist.")";
                                mysql_query($que2,$db);
                            }
