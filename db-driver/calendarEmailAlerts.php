<?php
	...
	while($drow=mysql_fetch_row($dres))
	{
		...
		$uque="SELECT users.username, timezone.phpvar, orgsetup.notify_email, orgsetup.notify_val,su.collaboration 
				FROM users 
		        ...
		        WHERE users.usertype!='' AND users.status!='DA' AND orgsetup.notify_email!=''";
		$ures=mysql_query($uque,$db);
		while($urow=mysql_fetch_row($ures))
		{
			$collaborationVal = $urow[4];
