...
require("ConvertCharsetTableList.php");
$Unicoderealpath=realpath(dirname(__FILE__))."/UnicodeTables/";
$PATH_TO_CLASS =$Unicoderealpath;
...
//calling convercharset class
function ConvertCharset_data($charset,$data)
{
	...
	if($charset!='' && $charset!=CONVERT_DEFAULT_MAIL_CHAR && in_array($charset,$CharSet_mail_array))
	{
		...
		$ConvertCharset_Obj=new ConvertCharset($charset,CONVERT_DEFAULT_MAIL_CHAR,false);
		return $ConvertCharset_Obj->Convert($data);
	}
	else
	{
		return $data;
