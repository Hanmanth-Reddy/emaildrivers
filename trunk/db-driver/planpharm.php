<?php
	...
	$dque="select capp_info.comp_id from company_info LEFT JOIN capp_info ON capp_info.sno=company_info.sno where company_info.status='ER' AND capp_info.comp_id='planpharm' ".$version_clause;
	...
	while($drow=mysql_fetch_row($dres))
	{
		...
		$que="select external_mail.imaddress,external_mail.import,external_mail.account,external_mail.passwd,external_mail.lcopy,external_mail.username,external_mail.sno,external_uidls.uidls,external_mail.imsslchk,external_mail.mtype,external_mail.last_rdate,external_mail.luidl,if(external_mail.folder is NULL,'inbox',external_mail.folder),external_mail.host_exchange,external_mail.stime,external_mail.lcount from external_mail LEFT JOIN users ON external_mail.username=users.username LEFT JOIN external_uidls ON external_uidls.extsno=external_mail.sno where external_mail.sno='12'";
